...
public:
	...
	//10.6 buff类附加
	void addBuff(Buff&);// 为一个随从添加一个Buff
	vector<Buff> _cardbuff;// 记录随从身上的buff
	void deleteBuffFromResource(int resource);// 删除来源类buff
	void reduceBuffTimes();// 减少所有buff持续时间
	void clearBuff();// 删除持续时间为0的buff
