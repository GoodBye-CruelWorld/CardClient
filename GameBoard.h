...
public:
	...
	*@camp:目标阵营
	*@delay:延迟执行时间
	*@remark:addCard是凭空产生卡牌,与cardTransfer不同.
	*/
	...
	*@cardAttack
	*@brief 集成的攻击函数.
	*@param srcOrder 攻击方在战斗池的位置.
	*@param srcCamp 攻击方的阵营.
	*@param srcHealth 攻击方攻击完后的生命值
	*@param destOrder 被攻击方在战斗池的位置.
	*@param destCamp 被攻击方的阵营.
	*@param destHealth 被攻击方攻击完后的生命值
	*@remark hero Order=-1时代表hero.
	*/
...
public:
	...
	void setHeroAttack(int camp,int value);
	/************************************************与BoardEffect有关的方法集****************************************/
public:
	/*快捷的增加特效方式*/
